...
Пример операторов:
*ReplicaSet* -- позволяет указывать необходимов количество подов, контроллеры следят за тем, чтобы их количество сохранялось.
*StatefulSet* -- включает в себя функционал ReplicaSet а также расширяет его для stateful приложений требованиями к постоянному хранилищу данных и статическому IP адресу.
...
Если на другой ноде нет ресурсов для размещения пода.
