...
*1 user* - количество пользователей авторизованнв     системе.

*load average: 0,78, 1,48, 1,10* - параметр средней  нагрузки   на систему за период времени 1 минута, минут,    15 минут.
...
*0 zombie* - количество зомби-процессов, которые  завершили   своё выполнение, но присутствующие системе,    чтобы дать   родительскому процессчитать  свой код    завершения.

| Параметр | Описание |
|-------------------------------|   ------------------------------------------------------ --   --  -----------------------------------|
| us (user) | Использование процессорпользовательским      процессами |
| sy (system) | Использование процессосистемным       процессами |
| ni (nice) | Использование процессора процессас       измененным приоритетом с помощью команды nice |
| id (idle) | Простой процессора. Можно сказать, чтэто      свободные ресурсы |
| wa (IO-wait) | Говорит о простое, связанным вводом/     выводом |
| hi (hardware interrupts) | Показывает сколько    процессорного времени было потрачено нобслуживание      аппаратного прерывания |
| si (software interrupts) | Показывает сколько    процессорного времени было потрачено нобслуживание      софтверного прерывания |
| st (stolen by the hypervisor) | Показываесколько      процессорного времени было «украденогипервизором |

KiB Mem - количество оперативной памятикибибайтах     (кратно   1024):
*7106404 total* -- всего доступно оперативной памятв      системе,
...
*KiB Swap* - количество swap-памяти в кибибайта(кратно      1024), которые выделено на диске:
...
6. Что такое процесс-зомби и процесс-сирота? Можно ли самостоятельно сделать зомби?
...
9. Какие DNS записи бывают? Что такое DKIM, DMARC, PTR?
...
10. Что такое RoundRobin DNS как работает?
...
11. Как настроить master-slave репликацию в mysql?
...
12. Представлен вывод команды `free`.
```
$ free -m
...
13. Вы вводите в строке браузера yandex.ru. Опишите процесс от нажатия клавиши до загрузки страницы.
...
4. Расскажите, что произойдёт при вводе команды: `./sh.sh !? 2 > 1 2 > &1 1 < 3`
...
5. Чем terraform отличается от ansible?
...
6. Имеете ли вы опыт построения отказоустойчивых решений с БД MySQL, PostgreSQL? Если да, какие решения использовали?

7. Имеете ли вы опыт построения и/или сопровождения высоконагруженных веб-проектов? Если да, опишите ваш опыт.

8. Работа со списками (добавить, удалить, найти определенный, етс.) в ansible?

9. Как, по какому критерию выбрать балансировщик из haproxy, httpd, nginx?
...
## Из опыта собеседований

- "Открываешь страницу сайта и видишь 500, как будешь исправлять?"
Каждый мой ответ заканчивался вопросом "Ну хорошо. <твой вариант> не сработает, что будешь делать дальше?". Я накидал вариантов 15 кажись и потом они (варианты) закончились. Наверное, тут не было правильного ответа, просто проверяли возможность к траблшутингу.
- Но меня спросили и про то, как организован коллокейшн, пиринговые центры типа мскикс, бгп, la, https, ансибль и терраформэ.
- На крайнем собеседовании спросили, вот про такое: `$1 #! ## ?? !? $$` и что-то ещё было.
- Например, беря задачу избеклога не просто делать задачу из беклога, а понимать, какой профит даст это бизнесу. И приоритизировать задачи по такому же приниципу - сначала бапки бизнесу, потом вкусняшки всякие. Я вот на каждом собеседование такое говорю и они говорят, ну да, у нас именно так, начинаешь работать, а там не так или не совсем так или через #опу.
- В госвакухах частое явление (как и в тендерах) заточка под своих. Они вывешивают заранее идиотские вакансии, долго "ищут", а потом всё равно берут кума/свата/брата, который изначально там и должен был находиться. Потому что если госухе нужно что-то более-менее рабочее, они просто закажут своей (часто карманной) конторе-аутсорсеру. Это удобней по многим критериям. Поэтому идти в госуху на собес можно только от отчаянья или если заняться нечем.
- Я вот тоже "вброшу" про ЛМ (как-то собеседовался): про стажировку не говорили, зато в конце собеса выяснилось, что вилка на 50к ниже :(
- Ну я весной ходил на собесы. Кому не нужна теория, тем нужен вэб. Кому не нужне вэб, тем надо паттерны, SOLID и многопоточность  + высоконагруженные приложения.
- Был тех. собес: GIL, мультипроцессорность, асинхронщина, даже проблему колизий в словарях -забыл/не знал... Самое что обидное, в одном подтянешь, а на другом собесе будет все другое (код на скорость писать например) и снова завалишь.
- Тут на собесе два странных вопроса были :) 1. Что быстрее при загрузке файла http, https или ftp? 2. Вы согласны с тем, что в гит можно замерджить без доступа в Интернет?
